...
/// ViewModel untuk HPP Calculator (menggunakan Cubit sebagai ViewModel pattern)
class HppCalculatorViewModel extends Cubit<HppCalculatorState> {
  final _uuid = const Uuid();

  HppCalculatorViewModel() : super(const HppCalculatorInitial());
  ...
  void calculateBep({
    ...
    try {
      final bepAnalysis = BepAnalysis(
        ...
        produksiBulanan: produksiBulanan,
      );
  ...
  /// Reset semua kalkulasi
  void reset() {
    emit(const HppCalculatorInitial());
  }

  /// Update profit margin pada kalkulasi yang sudah ada
